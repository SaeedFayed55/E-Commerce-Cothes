  ...
  <div>
    ...
    >
      <div class="confirmation-content">
        <i class="pi pi-exclamation-triangle p-mr-3" style="font-size: 2rem" />
        <span v-if="product"
          >Are you sure you want to confirm the selected products?</span
        >
      </div>
      <template #footer>
        <Button
...
export default {
  ...
  computed: {
    ...mapGetters(["getConfirmedShirts" , "UserOrders"]),
    orderDate() {
      const monthNames = [
        "January",
        "February",
  ...
  methods: {
    ...
    deleteProduct() {
      ...
      this.product.shippedDate = this.orderDate
      this.$store.dispatch("confirmShippingShirt", this.product);
    ...
    findIndexById(id) {
      let index = -1;
      for (let i = 0; i < this.getConfirmedShirts.length; i++) {
        if (this.getConfirmedShirts[i].id === id) {
          index = i;
          break;
        }
      }
    ...
    deleteSelectedProducts() {
      for (const prod of this.selectedProducts) {
          prod.status = "shipped"
          prod.shippedDate = this.orderDate
        axios.post("/shipped", prod).then((res) => {
         
          axios.post('/shippedIDs' , { id: prod.id , shippedDate : prod.shippedDate})
          axios.delete(`/confirmedShirts/${prod.id}`).then((res) => {
            console.log(res);
