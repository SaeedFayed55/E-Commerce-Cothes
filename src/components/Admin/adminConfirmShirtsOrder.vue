...
export default {
  ...
  computed: {
    ...mapGetters(["getPendingOrderG","UserOrders"]),
    orderDate() {
      const monthNames = [
        "January",
        "February",
  ...
  methods: {
    ...
    deleteProduct() {
      this.product.status = "cofirmed";
      this.product.confirmDate = this.orderDate;
      this.$store.dispatch("confirmOrderedShirt", this.product );
      ...
      if (this.updatedShirt.id) {
        this.$set(
          ...
          this.cart,
          axios.put(`/originaluserOrder/${this.updatedShirt.id}`, this.updatedShirt).then((res) => {
    ...
    findIndexById(id) {
      let index = -1;
      for (let i = 0; i < this.getPendingOrderG.length; i++) {
        if (this.getPendingOrderG[i].id === id) {
          index = i;
          break;
        }
      }
    ...
    deleteSelectedProducts() {
      for (const prod of this.selectedProducts) {
        ...
        prod.confirmDate = this.orderDate
        axios.post("/confirmedShirts", prod).then((res) => {
           axios.post('/confirmedIDs' , {id : prod.order.id , confirmDate : prod.confirmDate}).then(res => {
              axios.delete(`/pending/${prod.id}`).then((res) => {
            this.$store.dispatch("getPendingOrder");
            this.$store.dispatch("getShippedShirtss");
            this.$store.dispatch("getConfirmedShirtss");
            console.log(res);
            this.$toast.add({
              severity: "success",
              summary: "Successful",
