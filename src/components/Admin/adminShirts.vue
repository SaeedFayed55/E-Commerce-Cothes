<template>
  <div>
      ...
      >
        ...
        <Column header="Image">
          <template #body="slotProps">
            <img
              :src="require('@/assets/shirtsMen/' + slotProps.data.imageSrc)"
    ...
    >
      <img
        :src="require('@/assets/shirtsMen/' + product.imageSrc)"
        :alt="product.image"
        class="product-image"
...
export default {
  ...
  methods: {
      ...
      if (
        ...
        if (this.product.id) {
          this.$set(
            ...
            this.product,
            axios
              .put(`/shirts/${this.product.id}`, this.product)
              .then((res) => {
          ...
          axios.post("/shirts", this.product).then((res) => {
            this.$store.dispatch("initShirts");
            this.$toast.add({
              severity: "success",
              summary: "Successful",
    ...
    deleteProduct() {
      this.$store.dispatch("deleteShirt", this.product);
      this.$store.dispatch("initShirts");
    ...
    findIndexById(id) {
      let index = -1;
      for (let i = 0; i < this.getAllShirts.length; i++) {
        if (this.getAllShirts[i].id === id) {
          index = i;
          break;
        }
      }
    ...
    deleteSelectedProducts() {
      for (const prod of this.selectedProducts) {
        axios.delete(`/shirts/${prod.id}`).then((res) => {
          console.log(res);
          this.$store.dispatch("initShirts");
          this.$toast.add({
            severity: "success",
